...
void CriticalAngle(const size_t &Layer_num, QVector<LayerClass> &layerspecs) {
  ...
  for (int i = 1; i <= Layer_num; ++i) {
    ...
    layerspecs[i].layer->cos_crit_up =
        (n1 > n2) ? sqrt(1.0 - n2 * n2 / n1 * n1) : 0.0;

    n2 = layerspecs[i + 1].layer->rfct_index;

    layerspecs[i].layer->cos_crit_down =
        (n1 > n2) ? sqrt(1.0 - n2 * n2 / n1 * n1) : 0.0;
  }
}
...
void Sum2D_DiffReflect(const InputClass &In, OutClass &Out) {
  ...
  for (ir = 0; ir < nr; ir++) {
    sum = 0.0;
    for (ia = 0; ia < na; ia++)
      sum += Out.out->diff_reflect_2d[ir][ia];
  ...
  sum = 0.0;
  for (ir = 0; ir < nr; ir++)
    sum += Out.out->diff_reflect_rdl[ir];
...
void Sum2D_AbsProb(const InputClass &In_Parm, OutClass &Out_Ptr) {
  ...
  for (iz = 0; iz < nz; iz++) {
    sum = 0.0;
    for (ir = 0; ir < nr; ir++)
      sum += Out_Ptr.out->abs_prob_rz[ir][iz];
  ...
  for (iz = 0; iz < nz; iz++) {
    ...
    Out_Ptr.out->abs_prob_layer[IndexToLayer(iz, In_Parm)] +=
        Out_Ptr.out->abs_prob_z[iz];
  }
...
void Sum2D_TotalTrans(const InputClass &In_Parm, OutClass &Out_Ptr) {
  ...
  for (ir = 0; ir < nr; ir++) {
    sum = 0.0;
    for (ia = 0; ia < na; ia++)
      sum += Out_Ptr.out->total_trans_2d[ir][ia];
  ...
  sum = 0.0;
  for (ir = 0; ir < nr; ir++)
    sum += Out_Ptr.out->total_trans_rdl[ir];
